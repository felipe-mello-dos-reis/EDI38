...
phi = 0.032; % m
N = 4; % Numero de barras de aco
b = 0.20; %m
h = 0.70; %m
A_st = N*pi*phi*phi/4;
M_d = 550000*1.4*1e-6; %MN*m tinha que ser dado
...
while ((x_right-x_left) > tol*d) && (i < i_max)  && (abs(f_x) > tol)
    i = i+1;
    f_x_left = F(x_left,sigma_cd,b,d,x_lim,n,epslon_c2,epslon_cu,epslon_yd,f_yd,A_st);
    x = (x_right+x_left)/2; 
    f_x = F(x,sigma_cd,b,d,x_lim,n,epslon_c2,epslon_cu,epslon_yd,f_yd,A_st);
    if (f_x*f_x_left < 0)
        x_right = x;
    else
        x_left = x;
...
M_d_max = (Rcc(x,sigma_cd,b,d,x_lim,n,epslon_c2,epslon_cu)*d - Rcca(x,sigma_cd,b,d,x_lim,n,epslon_c2,epslon_cu));
fprintf("M_d_max = %f kN.m\n", M_d_max*1e3) % esse ja eh o majorado, precisa dividir por 1.4
...
%% Funcao objetivo
function f = F(x,sigma_cd,b,d,x_lim,n,epslon_c2,epslon_cu,epslon_yd,f_yd,A_st)
f = Rcc(x,sigma_cd,b,d,x_lim,n,epslon_c2,epslon_cu) - Rst(x,d,f_yd,epslon_c2,epslon_cu,epslon_yd,A_st);
end
%% Area de aco
